...
public class BPTrend{
        ...
        try{
            ...
            while(true){
                ...
                if(line == null){  
                    ...
                    weather.when = format.parse(wordsArray[0]);
                    weather.pressure = Float.valueOf(wordsArray[2]);
                    weather.humidity = Float.valueOf(wordsArray[4]);
                    collectedData.add(weather);
                }
            }
            buf.close();
        }catch(Exception e){
    ...
    public String doCalc( String from, String to){
        ...
        if (slope > 0)
        {
        	result += "Fair and Sunny weather expected.";
        }
        
        else if (slope < 0)
        {
        	result += "Stormy weather expected.";
        }
        
        else
        {
        	result += "Current conditions should persist.";
        }
